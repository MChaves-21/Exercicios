import { Handler } from "express";//Handler é usado para definir o tipo das funções que processam as requisições HTTP
import { prisma } from "../database";
import { HttpError } from "../errors/HttpError";
import {CreateCampaignRequestSchema, UpdateCampaignRequestSchema } from "./schemas/CampaingRequestSchema";
...
export class CampaignsController {
    ...
    show: Handler = async (req, res, next) => {
      try {
        ...
        if (!campaign) throw new HttpError(404, "camapnha não encontrada")
        res.json(campaign)
      } catch (error) {
        next(error)
      }
